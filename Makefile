...
minisynd: src/main.nim
	nim c -d:debug --nimcache:nimcache/debug -p:../nico -o:$@ --threads:on --tlsEmulation:off -d:gif $<

minisyn: src/main.nim
	nim c -d:release --nimcache:nimcache/release -p:../nico -o:$@ --threads:on --tlsEmulation:off -d:gif $<
...
osx: src/main.nim
	${NIMC} c -p:../nico -d:release -d:osx --threads:on --tlsEmulation:off -o:minisyn.app/Contents/MacOS/minisyn src/main.nim
...
linux64: $(SOURCES)
	${NIMC} c -p:../nico -d:release --threads:on --tlsEmulation:off -o:linux/${APPNAME}.x86_64 src/main.nim

linux32: $(SOURCES)
	${NIMC} c -p:../nico -d:release -d:linux32 --threads:on --tlsEmulation:off -o:linux/${APPNAME}.x86 src/main.nim
